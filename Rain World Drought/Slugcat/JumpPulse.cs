...
class JumpPulse : CosmeticSprite
{
    ...
    public override void InitiateSprites(RoomCamera.SpriteLeaser sLeaser, RoomCamera rCam)
    {
        ...
        for(int i = 0; i < segs; i++)
        {
            dir.x = 0.5f * width  * Mathf.Cos(i * (Mathf.PI * 2f / segs));
            dir.y = 0.5f * height * Mathf.Sin(i * (Mathf.PI * 2f / segs));
            float scl = 1f + thickness / width;
            ring.vertices[i * 2 + 0].Set(dir.x, dir.y);
            ring.vertices[i * 2 + 1].Set(dir.x * scl, dir.y * scl);
        }
